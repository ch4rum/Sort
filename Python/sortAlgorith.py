...
class SortAlg:
  ...
  ,                                    :      ,W,          
    ...
    def insertion_sort(self: 'SortAlg', numbers: list[int], p1: log.progress) -> list[int]:
        ...
        At each iteration, it takes an unsorted element, compares it with the sorted elements and 
    ...
    def quickSort(self: 'SortAlg', numbers: list[int], p1: log.progress) -> list[int]:
        ...
        an item as a pivot and rearrange the list so that items smaller than the pivot are on the left and items
        larger than the pivot are on the right. 
        ...
        return self._quickSortHelp(numbers,0,len(numbers)-1, p1)

    def _partition(self: 'SortAlg', numbers: list[int], low: int, high: int, p1: log.progress) -> int:
    ...
    def _quickSortHelp(self: '', numbers: list[int], low: int, high: int, p1: log.progress) -> list[int]:

            if low < high: #1
                pi = self._partition(numbers,low,high,p1)
                self._quickSortHelp(numbers,low,pi-1,p1)
                self._quickSortHelp(numbers,pi+1,high,p1)
            return numbers
